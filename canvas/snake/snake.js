(() => {
  ...
  let controller; let display; let
    game;
  ...
  display = {
    ...
    render() {
      ...
      for (let index = 0; index < game.world.map.length; index++) {
        ...
        const graphic = this.graphics[game.world.map[index]].canvas;
    ...
    loop(time_stamp) {
      ...
      if (time_stamp >= game.accumulated_time + game.time_step) {
        ...
        if (game.snake.vector_x != 0 || game.snake.vector_y != 0) {
          ...
          const tail_index = game.snake.segment_indices.pop();// remove the tail
  ...
  for (const object in display.graphics) {
